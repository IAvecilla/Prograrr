...
-opaque next(MXE, MXF) :: {continue,
        MXE,
        gleam@option:option(gleam@erlang@process:selector(MXF))} |
    normal_stop |
...
-opaque builder(MXG, MXH) :: {builder,
        integer(),
        fun((gleam@http@request:request(MXG)) -> gleam@http@response:response(MXH)),
...
-type websocket_message(MXI) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, MXI}.
...
-spec continue(MXJ) -> next(MXJ, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 58).
-spec with_selector(next(MXN, MXO), gleam@erlang@process:selector(MXO)) -> next(MXN, MXO).
...
-spec convert_next(next(MYC, MYD)) -> mist@internal@next:next(MYC, MYD).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(MZE)) -> gleam@http@response:response(MZG))
) -> builder(MZE, MZG).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(MZK, MZL), integer()) -> builder(MZK, MZL).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), MZQ),
    integer(),
    gleam@http@response:response(MZQ)
) -> builder(mist@internal@http:connection(), MZQ).
...
-spec bind(builder(NAC, NAD), binary()) -> builder(NAC, NAD).
...
-spec with_ipv6(builder(NAI, NAJ)) -> builder(NAI, NAJ).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(NAO, NAP), binary(), binary()) -> builder(NAO, NAP).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(NBE)
) -> {ok, websocket_message(NBE)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((NBK, websocket_message(NBL), mist@internal@websocket:websocket_connection()) -> next(NBK, NBL)),
    fun((mist@internal@websocket:websocket_connection()) -> {NBK,
        gleam@option:option(gleam@erlang@process:selector(NBL))}),
    fun((NBK) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(NBZ)) -> {ok,
            gleam@otp@actor:initialised(NCB, NBZ, any())} |
        {error, binary()}),
    fun((NCB, NBZ, s_s_e_connection()) -> gleam@otp@actor:next(NCB, NBZ))
) -> gleam@http@response:response(response_data()).
