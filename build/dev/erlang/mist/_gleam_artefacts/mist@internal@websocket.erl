...
-type valid_message(MIU) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, MIU}.

-type websocket_message(MIV) :: {valid, valid_message(MIV)} | invalid.
...
-type handler_message(MIW) :: {internal, gramps@websocket:frame()} | {user, MIW}.

-type websocket_state(MIX) :: {websocket_state,
        bitstring(),
        MIX,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(MKD))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(MKD))).
...
-spec apply_frames(
    ...
    fun((MJT, handler_message(MJU), websocket_connection()) -> mist@internal@next:next(MJT, MJU)),
    websocket_connection(),
    mist@internal@next:next(MJT, websocket_message(MJU)),
    fun((MJT) -> nil)
) -> mist@internal@next:next(MJT, websocket_message(MJU)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {MJG,
        gleam@option:option(gleam@erlang@process:selector(MJH))}),
    fun((MJG) -> nil),
    fun((MJG, handler_message(MJH), websocket_connection()) -> mist@internal@next:next(MJG, MJH)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(MJH)))} |
    {error, nil}.
