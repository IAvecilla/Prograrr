...
-type action(GJU) :: stop | {continue, GJU, fun(() -> action(GJU))}.

-opaque yielder(GJV) :: {yielder, fun(() -> action(GJV))}.

-type step(GJW, GJX) :: {next, GJW, GJX} | done.

-type chunk(GJY, GJZ) :: {another_by,
        list(GJY),
        GJZ,
        GJY,
        fun(() -> action(GJY))} |
...
-type sized_chunk(GKA) :: {another, list(GKA), fun(() -> action(GKA))} |
    {last, list(GKA)} |
    no_more.
...
-spec unfold_loop(GKI, fun((GKI) -> step(GKJ, GKI))) -> fun(() -> action(GKJ)).
...
-spec unfold(GKD, fun((GKD) -> step(GKE, GKD))) -> yielder(GKE).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> GKN)) -> yielder(GKN).
repeatedly(F) ->
...
-spec from_list(list(GKR)) -> yielder(GKR).
from_list(List) ->
...
-spec transform(yielder(GLB), GLD, fun((GLD, GLB) -> step(GLE, GLD))) -> yielder(GLE).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(GLL)), fun((GLN, GLL) -> GLN), GLN) -> GLN.
...
-spec fold(yielder(GLI), GLK, fun((GLK, GLI) -> GLK)) -> GLK.
...
-spec to_list(yielder(GLQ)) -> list(GLQ).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(GLT)) -> step(GLT, yielder(GLT)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(GMB)), integer()) -> fun(() -> action(GMB)).
...
-spec take(yielder(GLY), integer()) -> yielder(GLY).
...
-spec drop_loop(fun(() -> action(GMH)), integer()) -> action(GMH).
...
-spec map(yielder(GMK), fun((GMK) -> GMM)) -> yielder(GMM).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(GMS), yielder(GMU), fun((GMS, GMU) -> GMW)) -> yielder(GMW).
...
-spec append_loop(fun(() -> action(GNI)), fun(() -> action(GNI))) -> action(GNI).
...
-spec append(yielder(GNE), yielder(GNE)) -> yielder(GNE).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(GNQ)))) -> action(GNQ).
...
-spec flatten(yielder(yielder(GNM))) -> yielder(GNM).
flatten(Yielder) ->
...
-spec concat(list(yielder(GNU))) -> yielder(GNU).
concat(Yielders) ->
...
-spec flat_map(yielder(GNY), fun((GNY) -> yielder(GOA))) -> yielder(GOA).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(GOG)), fun((GOG) -> boolean())) -> action(GOG).
...
-spec filter(yielder(GOD), fun((GOD) -> boolean())) -> yielder(GOD).
...
-spec filter_map(yielder(GOJ), fun((GOJ) -> {ok, GOL} | {error, any()})) -> yielder(GOL).
...
-spec cycle(yielder(GOX)) -> yielder(GOX).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(GPF)), fun((GPF) -> boolean())) -> {ok, GPF} |
    {error, nil}.
...
-spec find(yielder(GPB), fun((GPB) -> boolean())) -> {ok, GPB} | {error, nil}.
...
-spec find_map(yielder(GPJ), fun((GPJ) -> {ok, GPL} | {error, any()})) -> {ok,
        GPL} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(GQC)), integer()) -> fun(() -> action({GQC,
    integer()})).
...
-spec index(yielder(GPZ)) -> yielder({GPZ, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(GQF, fun((GQF) -> GQF)) -> yielder(GQF).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(GQK)), fun((GQK) -> boolean())) -> fun(() -> action(GQK)).
...
-spec take_while(yielder(GQH), fun((GQH) -> boolean())) -> yielder(GQH).
...
-spec drop_while_loop(fun(() -> action(GQQ)), fun((GQQ) -> boolean())) -> action(GQQ).
...
-spec drop_while(yielder(GQN), fun((GQN) -> boolean())) -> yielder(GQN).
...
-spec scan_loop(fun(() -> action(GQX)), fun((GQZ, GQX) -> GQZ), GQZ) -> fun(() -> action(GQZ)).
...
-spec scan(yielder(GQT), GQV, fun((GQV, GQT) -> GQV)) -> yielder(GQV).
...
-spec zip_loop(fun(() -> action(GRG)), fun(() -> action(GRI))) -> fun(() -> action({GRG,
    GRI})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(GRB), yielder(GRD)) -> yielder({GRB, GRD}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(GRV)), fun((GRV) -> GRX), GRX, list(GRV)) -> chunk(GRV, GRX).
...
-spec chunk_loop(fun(() -> action(GRQ)), fun((GRQ) -> GRS), GRS, GRQ) -> action(list(GRQ)).
...
-spec chunk(yielder(GRL), fun((GRL) -> any())) -> yielder(list(GRL)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(GSJ)), integer(), list(GSJ)) -> sized_chunk(GSJ).
...
-spec sized_chunk_loop(fun(() -> action(GSF)), integer()) -> fun(() -> action(list(GSF))).
...
-spec sized_chunk(yielder(GSB), integer()) -> yielder(list(GSB)).
...
-spec intersperse_loop(fun(() -> action(GSQ)), GSQ) -> action(GSQ).
...
-spec intersperse(yielder(GSN), GSN) -> yielder(GSN).
...
-spec any_loop(fun(() -> action(GSV)), fun((GSV) -> boolean())) -> boolean().
...
-spec any(yielder(GST), fun((GST) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(GSZ)), fun((GSZ) -> boolean())) -> boolean().
...
-spec all(yielder(GSX), fun((GSX) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(GTP) -> fun((gleam@option:option(list(GTP))) -> list(GTP)).
...
-spec group_updater(fun((GTH) -> GTI)) -> fun((gleam@dict:dict(GTI, list(GTH)), GTH) -> gleam@dict:dict(GTI, list(GTH))).
group_updater(F) ->
...
-spec group(yielder(GTB), fun((GTB) -> GTD)) -> gleam@dict:dict(GTD, list(GTB)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(GTT), fun((GTT, GTT) -> GTT)) -> {ok, GTT} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(GTX)) -> {ok, GTX} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(GUF) -> yielder(GUF).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(GUL)), fun(() -> action(GUL))) -> action(GUL).
...
-spec interleave(yielder(GUH), yielder(GUH)) -> yielder(GUH).
...
-spec fold_until_loop(
    fun(() -> action(GUT)),
    fun((GUV, GUT) -> gleam@list:continue_or_stop(GUV)),
    GUV
) -> GUV.
...
-spec fold_until(
    yielder(GUP),
    GUR,
    fun((GUR, GUP) -> gleam@list:continue_or_stop(GUR))
) -> GUR.
...
-spec try_fold(yielder(GUX), GUZ, fun((GUZ, GUX) -> {ok, GUZ} | {error, GVA})) -> {ok,
        GUZ} |
    {error, GVA}.
...
-spec first(yielder(GVN)) -> {ok, GVN} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(GVR), integer()) -> {ok, GVR} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(GVZ), fun((GVZ) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(GWC, fun(() -> yielder(GWC))) -> yielder(GWC).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(GWF), GWF) -> yielder(GWF).
