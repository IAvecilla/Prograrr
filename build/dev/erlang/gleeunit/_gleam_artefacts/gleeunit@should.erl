...
-spec be_ok({ok, RTE} | {error, any()}) -> RTE.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, RTJ}) -> RTJ.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(RTM)) -> RTM.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
