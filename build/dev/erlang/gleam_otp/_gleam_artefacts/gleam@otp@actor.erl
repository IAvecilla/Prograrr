...
-type message(EYW) :: {message, EYW} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(EYX, EYY) :: {continue,
        EYX,
        gleam@option:option(gleam@erlang@process:selector(EYY))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(EYZ, EZA) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        EYZ,
        gleam@erlang@process:selector(message(EZA)),
        gleam@otp@system:debug_state(),
        fun((EYZ, EZA) -> next(EYZ, EZA))}.

-type started(EZB) :: {started, gleam@erlang@process:pid_(), EZB}.

-opaque initialised(EZC, EZD, EZE) :: {initialised,
        EZC,
        gleam@option:option(gleam@erlang@process:selector(EZD)),
        EZE}.

-opaque builder(EZF, EZG, EZH) :: {builder,
        fun((gleam@erlang@process:subject(EZG)) -> {ok,
                initialised(EZF, EZG, EZH)} |
            {error, binary()}),
        integer(),
        fun((EZF, EZG) -> next(EZF, EZG)),
        gleam@option:option(gleam@erlang@process:name(EZG))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(EZI) :: {ack, {ok, EZI} | {error, binary()}} |
...
-spec continue(EZN) -> next(EZN, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(EZZ, FAA), gleam@erlang@process:selector(FAA)) -> next(EZZ, FAA).
...
-spec initialised(FAG) -> initialised(FAG, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(FAL, any(), FAN),
    gleam@erlang@process:selector(FAR)
) -> initialised(FAL, FAR, FAN).
...
-spec returning(initialised(FAW, FAX, any()), FBC) -> initialised(FAW, FAX, FBC).
...
-spec new(FBG) -> builder(FBG, FBH, gleam@erlang@process:subject(FBH)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(FBM)) -> {ok, initialised(FBO, FBM, FBP)} |
        {error, binary()})
) -> builder(FBO, FBM, FBP).
...
-spec on_message(builder(FBY, FBZ, FCA), fun((FBY, FBZ) -> next(FBY, FBZ))) -> builder(FBY, FBZ, FCA).
...
-spec named(builder(FCJ, FCK, FCL), gleam@erlang@process:name(FCK)) -> builder(FCJ, FCK, FCL).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(FCY))) -> gleam@erlang@process:selector(message(FCY)).
...
-spec receive_message(self(any(), FCU)) -> message(FCU).
...
-spec initialise_actor(
    builder(any(), any(), FDR),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, FDR} | {error, binary()})
...
-spec start(builder(any(), any(), FEE)) -> {ok, started(FEE)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(FEL), FEL) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(FEN),
    integer(),
    fun((gleam@erlang@process:subject(FEP)) -> FEN)
) -> FEP.
