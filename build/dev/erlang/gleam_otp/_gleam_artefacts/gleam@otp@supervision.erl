...
-type child_specification(FSB) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(FSB)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(FSC)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FSC).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(FSH)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FSH).
supervisor(Start) ->
...
-spec significant(child_specification(FSM), boolean()) -> child_specification(FSM).
...
-spec timeout(child_specification(FSP), integer()) -> child_specification(FSP).
timeout(Child, Ms) ->
...
-spec restart(child_specification(FSS), restart()) -> child_specification(FSS).
...
-spec map_data(child_specification(FSV), fun((FSV) -> FSX)) -> child_specification(FSX).
