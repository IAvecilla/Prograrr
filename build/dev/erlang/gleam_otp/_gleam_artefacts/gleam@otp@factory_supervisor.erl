...
-opaque supervisor(FUP, FUQ) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(FUP, FUQ))}.

-type message(FUR, FUS) :: any() | {gleam_phantom, FUR, FUS}.

-opaque builder(FUT, FUU) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((FUT) -> {ok, gleam@otp@actor:started(FUU)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(FUT, FUU)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(FUV, FUW) :: {local,
        gleam@erlang@process:name(message(FUV, FUW))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(FUX) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, FUX}.
...
-type erlang_child_spec_property(FUY, FUZ) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((FUY) -> {ok, gleam@otp@actor:started(FUZ)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(FVA, FVB))) -> supervisor(FVA, FVB).
get_by_name(Name) ->
...
-spec named(builder(FVR, FVS), gleam@erlang@process:name(message(FVR, FVS))) -> builder(FVR, FVS).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(FWA, FWB), integer(), integer()) -> builder(FWA, FWB).
...
-spec timeout(builder(FWG, FWH), integer()) -> builder(FWG, FWH).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(FWM, FWN), gleam@otp@supervision:restart()) -> builder(FWM, FWN).
...
-spec start(builder(FWS, FWT)) -> {ok,
        gleam@otp@actor:started(supervisor(FWS, FWT))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(FXR, FXS)) -> gleam@otp@supervision:child_specification(supervisor(FXR, FXS)).
supervised(Builder) ->
...
-spec start_child(supervisor(FXY, FXZ), FXY) -> {ok,
        gleam@otp@actor:started(FXZ)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((FYW) -> {ok, gleam@otp@actor:started(FYX)} |
        {error, gleam@otp@actor:start_error()}),
    FYW
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), FYX, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((FVH) -> {ok, gleam@otp@actor:started(FVI)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(FVH, FVI).
...
-spec supervisor_child(
    fun((FVM) -> {ok, gleam@otp@actor:started(FVN)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(FVM, FVN).
