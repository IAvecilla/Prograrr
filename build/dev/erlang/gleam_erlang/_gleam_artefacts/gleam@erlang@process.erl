...
-opaque subject(EPL) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(EPL)}.

-type name(EPM) :: any() | {gleam_phantom, EPM}.

-type do_not_leak() :: any().

-type selector(EPN) :: any() | {gleam_phantom, EPN}.
...
-spec 'receive'(subject(EQL), integer()) -> {ok, EQL} | {error, nil}.
...
-spec receive_forever(subject(EQT)) -> EQT.
...
-spec selector_receive(selector(EQX), integer()) -> {ok, EQX} | {error, nil}.
...
-spec selector_receive_forever(selector(ERB)) -> ERB.
...
-spec map_selector(selector(ERD), fun((ERD) -> ERF)) -> selector(ERF).
map_selector(A, B) ->
...
-spec merge_selector(selector(ERH), selector(ERH)) -> selector(ERH).
merge_selector(A, B) ->
...
-spec select_map(selector(ERS), subject(ERU), fun((ERU) -> ERS)) -> selector(ERS).
...
-spec select(selector(ERO), subject(ERO)) -> selector(ERO).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> ESC)
) -> selector(ESC).
...
-spec select_other(selector(ESG), fun((gleam@dynamic:dynamic_()) -> ESG)) -> selector(ESG).
...
-spec deselect(selector(ERX), subject(any())) -> selector(ERX).
...
-spec select_specific_monitor(selector(ESS), monitor(), fun((down()) -> ESS)) -> selector(ESS).
...
-spec select_monitors(selector(ESV), fun((down()) -> ESV)) -> selector(ESV).
...
-spec select_trapped_exits(selector(ERL), fun((exit_message()) -> ERL)) -> selector(ERL).
...
-spec deselect_specific_monitor(selector(ESY), monitor()) -> selector(ESY).
...
-spec send_after(subject(ETU), integer(), ETU) -> timer().
...
-spec perform_call(
    subject(ETB),
    fun((subject(ETD)) -> ETB),
    fun((selector(ETD)) -> {ok, ETD} | {error, nil})
) -> ETD.
...
-spec call(subject(ETI), integer(), fun((subject(ETK)) -> ETI)) -> ETK.
...
-spec call_forever(subject(ETM), fun((subject(ETO)) -> ETM)) -> ETO.
