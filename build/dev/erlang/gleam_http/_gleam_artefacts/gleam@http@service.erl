...
-spec map_response_body(
    fun((ELB) -> gleam@http@response:response(ELC)),
    fun((ELC) -> ELE)
) -> fun((ELB) -> gleam@http@response:response(ELE)).
...
-spec prepend_response_header(
    fun((ELG) -> gleam@http@response:response(ELH)),
    binary(),
    binary()
) -> fun((ELG) -> gleam@http@response:response(ELH)).
...
-spec ensure_post(gleam@http@request:request(ELK)) -> {ok,
        gleam@http@request:request(ELK)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(ELT)) -> ELV)) -> fun((gleam@http@request:request(ELT)) -> ELV).
