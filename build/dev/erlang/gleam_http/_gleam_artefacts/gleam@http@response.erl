...
-type response(EHF) :: {response, integer(), list({binary(), binary()}), EHF}.
...
-spec set_header(response(EHU), binary(), binary()) -> response(EHU).
...
-spec prepend_header(response(EHX), binary(), binary()) -> response(EHX).
...
-spec set_body(response(any()), EIC) -> response(EIC).
...
-spec try_map(response(EHG), fun((EHG) -> {ok, EHI} | {error, EHJ})) -> {ok,
        response(EHI)} |
    {error, EHJ}.
...
-spec map(response(EIE), fun((EIE) -> EIG)) -> response(EIG).
...
-spec expire_cookie(response(EIP), binary(), gleam@http@cookie:attributes()) -> response(EIP).
