...
-spec set_header(request(DXL), binary(), binary()) -> request(DXL).
...
-spec prepend_header(request(DXO), binary(), binary()) -> request(DXO).
...
-spec set_body(request(any()), DXT) -> request(DXT).
set_body(Req, Body) ->
    {request,
...
-spec map(request(DXV), fun((DXV) -> DXX)) -> request(DXX).
...
-spec set_query(request(DYH), list({binary(), binary()})) -> request(DYH).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(DYL), gleam@http:method()) -> request(DYL).
...
-spec set_scheme(request(DYS), gleam@http:scheme()) -> request(DYS).
...
-spec set_host(request(DYV), binary()) -> request(DYV).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(DYY), integer()) -> request(DYY).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(DZB), binary()) -> request(DZB).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(DZE), binary(), binary()) -> request(DZE).
...
-spec remove_cookie(request(DZK), binary()) -> request(DZK).
