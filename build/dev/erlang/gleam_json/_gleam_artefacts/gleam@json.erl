...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(OPR)) -> {ok, OPR} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(OPL)) -> {ok, OPL} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(OPH)) -> {ok, OPH} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(OPX), fun((OPX) -> json())) -> json().
...
-spec array(list(OQB), fun((OQB) -> json())) -> json().
