...
-export_type([aggregator_error/0]).
...
-type aggregator_error() :: {jellyseerr_error,
        api@jellyseerr:jellyseerr_error()} |
    {sonarr_error, api@arr:arr_error()} |
    {radarr_error, api@arr:arr_error()} |
    {q_bit_error, api@qbittorrent:q_bit_error()}.

-file("src/api/aggregator.gleam", 115).
...
-file("src/api/aggregator.gleam", 128).
...
-file("src/api/aggregator.gleam", 141).
...
-file("src/api/aggregator.gleam", 151).
...
find_torrent_by_name(Torrents, Title) ->
    Lower_title = string:lowercase(Title),
    _pipe = Torrents,
    _pipe@2 = gleam@list:find(
        _pipe,
        fun(T) -> _pipe@1 = string:lowercase(erlang:element(3, T)),
            gleam_stdlib:contains_string(_pipe@1, Lower_title) end
    ),
    gleam@option:from_result(_pipe@2).

-file("src/api/aggregator.gleam", 163).
...
-file("src/api/aggregator.gleam", 52).
...
-file("src/api/aggregator.gleam", 23).
