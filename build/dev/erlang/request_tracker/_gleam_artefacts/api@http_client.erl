...
-export([error_message/1, get/2, get_with_cookie/3, post_with_cookie/3, post/3]).
...
-file("src/api/http_client.gleam", 89).
-spec add_headers(
...
-file("src/api/http_client.gleam", 95).
...
-file("src/api/http_client.gleam", 82).
?DOC(" Make an HTTP POST request (simple version)\n").
-spec post(binary(), binary(), list({binary(), binary()})) -> {ok, binary()} |
    {error, http_error()}.
post(Url, Body, Headers) ->
    case post_with_cookie(Url, Body, Headers) of
        {ok, Resp} ->
            {ok, erlang:element(2, Resp)};
