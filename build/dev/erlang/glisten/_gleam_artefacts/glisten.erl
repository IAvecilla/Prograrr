...
-type message(IVY) :: {packet, bitstring()} | {user, IVY}.
...
-type connection(IVZ) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(IVZ))}.

-opaque next(IWA, IWB) :: {continue,
        IWA,
        gleam@option:option(gleam@erlang@process:selector(IWB))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(IWC, IWD) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(IWD)) -> {IWC,
            gleam@option:option(gleam@erlang@process:selector(IWD))}),
        fun((IWC, message(IWD), connection(IWD)) -> next(IWC, message(IWD))),
        gleam@option:option(fun((IWC) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(IXD) -> next(IXD, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 151).
-spec with_selector(next(IXH, IXI), gleam@erlang@process:selector(IXI)) -> next(IXH, IXI).
...
?DOC(false).
-spec convert_next(next(IXW, IXX)) -> glisten@internal@handler:next(IXW, IXX).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(IYC, IYD), fun((IYD) -> IYG)) -> next(IYC, IYG).
...
-spec map_user_selector(gleam@erlang@process:selector(message(IYJ))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(IYJ)).
...
-spec convert_loop(
    fun((IYO, message(IYP), connection(IYP)) -> next(IYO, message(IYP)))
) -> fun((IYO, glisten@internal@handler:loop_message(IYP), glisten@internal@handler:connection(IYP)) -> glisten@internal@handler:next(IYO, glisten@internal@handler:loop_message(IYP))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(IYU)) -> {IYW,
        gleam@option:option(gleam@erlang@process:selector(IYU))})
) -> fun((glisten@internal@handler:connection(IYU)) -> {IYW,
    gleam@option:option(gleam@erlang@process:selector(IYU))}).
...
-spec new(
    fun((connection(IZC)) -> {IZE,
        gleam@option:option(gleam@erlang@process:selector(IZC))}),
    fun((IZE, message(IZC), connection(IZC)) -> next(IZE, message(IZC)))
) -> builder(IZE, IZC).
new(On_init, Loop) ->
...
-spec with_close(builder(IZL, IZM), fun((IZL) -> nil)) -> builder(IZL, IZM).
...
-spec with_pool_size(builder(IZR, IZS), integer()) -> builder(IZR, IZS).
...
?DOC(false).
-spec with_http2(builder(IZX, IZY)) -> builder(IZX, IZY).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(JAJ, JAK)) -> builder(JAJ, JAK).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(JAP, JAQ), binary(), binary()) -> builder(JAP, JAQ).
...
-spec bind(builder(JAD, JAE), binary()) -> builder(JAD, JAE).
