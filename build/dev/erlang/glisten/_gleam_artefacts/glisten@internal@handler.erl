...
-type message(IBX) :: {internal, internal_message()} | {user, IBX}.

-type loop_message(IBY) :: {packet, bitstring()} | {custom, IBY}.

-type loop_state(IBZ, ICA) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(ICA)),
        glisten@transport:transport(),
        IBZ}.

-type connection(ICB) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(ICB))}.

-type next(ICC, ICD) :: {continue,
        ICC,
        gleam@option:option(gleam@erlang@process:selector(ICD))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(ICE, ICF) :: {handler,
        glisten@socket:socket(),
        fun((ICE, loop_message(ICF), connection(ICF)) -> next(ICE, loop_message(ICF))),
        fun((connection(ICF)) -> {ICE,
            gleam@option:option(gleam@erlang@process:selector(ICF))}),
        gleam@option:option(fun((ICE) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(ICS) -> next(ICS, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(ICW, ICX), gleam@erlang@process:selector(ICX)) -> next(ICW, ICX).
...
?DOC(false).
-spec start(handler(any(), IDM)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(IDM)))} |
